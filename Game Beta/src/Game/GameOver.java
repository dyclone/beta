...
public class GameOver extends Game{
	...
	public void run() {
		...
		while(running){
			...
			render();
			if(count == 1){
				t.schedule(new LoseTask(), 1*1000);
				t.schedule(new LoseTask(), 2*1000);
				t.schedule(new LoseTask(), 3*1000);
				t.schedule(new ExitTask(), 4*1000);
				count = 0;
			}
...
//				System.out.println("FPS: " + frames + " TICKS: " + updates);
	...
	public void render(){
		...
		g.drawString("Game Over", this.getWidth()/2 - (this.getWidth()/26), this.getHeight()/2- (this.getHeight()/10));
		g.setFont(new Font("TimesRoman", Font.PLAIN, 18));
		g.drawString("Exiting game in: "+timeRemaining, this.getWidth()/2 - (this.getWidth()/26) - 20, this.getHeight()/2);
...
class LoseTask extends TimerTask{

	@Override
	public void run() {
		
		GameOver.setTime(GameOver.getTime()-1);
